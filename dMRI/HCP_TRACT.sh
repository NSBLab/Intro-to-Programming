...
RUN_STAGE_FIVE=0
# RUN_STAGE_SIX copies parcellations into the work directory
RUN_STAGE_SIX=0
...
# Set the name and location of the T1 and T2 images for use with ACT
ACT_T1="${PARENTDIR}/${SUBJECTID}/T1w/T1w_acpc_dc_restore_brain.nii.gz"
ACT_T2="${PARENTDIR}/${SUBJECTID}/T1w/T2w_acpc_dc_restore_brain.nii.gz"
...
#PARC_LIST="HCPMMP1ANDfslatlas20_acpc HCPMMP1_acpc aparc+first_acpc aparc+aseg_acpc custom200_acpc custom500_acpc custom500ANDfslatlas20_acpc custom200ANDfslatlas20_acpc aparc_acpc"
...
# Set to 1 if moving the data to mnt
MOVE_DATA=0

# Generate the DWI image for use
DWI_GEN=0
...
# SET SBTCH = 1 if using HCP_SETUP.sh to submit jobs
USING_SBATCH=1
...
if [ "${RUN_PREPARATION}" = 1 ]; then
	...
	if [ "${ACT_gen}" = 0 ]; then
		...
		if [ "${HCP}" = 1 ]; then
			...
			cp -Rfv ${ACT_T1} ${WORKDIR}/${ACT_T1_NAME}
			cp -Rfv ${ACT_T2} ${WORKDIR}
